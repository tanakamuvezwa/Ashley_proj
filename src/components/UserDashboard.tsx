...
const LoanStatusTracker: React.FC<{ loan: LoanRequest }> = ({ loan }) => {
 ...
 i <= step ? 'bg-emerald-500 text-slate-950' : ' text-slate-600'
 }`}>
 {i < step ? (
 ...
 <div className={`flex-1 h-0.5 mx-1 transition-all ${i < step ? 'bg-emerald-500' : ''}`} />
 )}
 </React.Fragment>
 ))}
 </div>
...
export const UserDashboard: React.FC<UserDashboardProps> = ({ currentUser, onNavigate }) => {
 ...
 {/* Header */}
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 pb-8 border-b ">
 <div>
 ...
 {/* Section Tabs */}
 <div className="flex space-x-1 mb-8 p-1 rounded-xl border w-fit">
 ...
 <div key={label} className="glass-card p-5 rounded-2xl border ">
 ...
 <div key={loan.id} className="p-4 rounded-xl border space-y-2">
 ...
 <button
 onClick={() => onNavigate('uber-loans')}
 className="glass-card p-5 rounded-2xl border hover:border-emerald-500/20 text-left cursor-pointer group transition"
 >
 ...
 className="glass-card p-5 rounded-2xl border hover:border-amber-500/20 text-left cursor-pointer group transition"
 >
 <div className="flex items-center justify-between mb-3">
 ...
 <div className="flex items-center space-x-2">
 <button onClick={fetchLoans} className="p-2 rounded-lg border text-slate-400 hover:text-white transition cursor-pointer">
 ...
 <div className="glass-card p-12 rounded-2xl border text-center space-y-4">
 ...
 <div className="space-y-4">
 {loans.map(loan => (
 <div key={loan.id} className="glass-card p-6 rounded-2xl border space-y-4">
 ...
 <div className="flex items-center space-x-2">
 <button onClick={fetchPitches} className="p-2 rounded-lg border text-slate-400 hover:text-white transition cursor-pointer">
 <RefreshCw className="w-3.5 h-3.5" />
 </button>
 <button
 onClick={() => onNavigate('projects')}
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 {pitches.map(pitch => (
 <div key={pitch.id} className="glass-card p-5 rounded-2xl border space-y-4">
 <div>
 ...
 <div className="grid grid-cols-3 gap-2 text-[9px] font-mono border-t pt-3">
