...
export const LoginModal: React.FC<LoginModalProps> = ({
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full glass-input pl-10 pr-${field === 'password' ? '10' : '4'} py-2.5 rounded-xl text-xs text-white bg-slate-950/40 transition-all outline-none border ${
        ...
        {field === 'password' && activeMode === 'register' && password.length > 0 && (
          <div className="mt-1.5 flex items-center space-x-2">
            <div className="flex space-x-0.5 flex-1">
              {[1,2,3,4,5].map(i => (
                <div key={i} className={`h-0.5 flex-1 rounded-full transition-all ${i <= pwStrength.score ? pwStrength.color : 'bg-slate-800'}`} />
              ))}
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="glass-card max-w-md w-full p-6 sm:p-8 rounded-3xl border border-white/10 relative shadow-2xl bg-slate-950 animate-in">

        {/* Close Button */}
        <button onClick={onClose} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-white rounded-full bg-slate-900 transition border border-white/5 cursor-pointer">
          <X className="w-4 h-4" />
        </button>
        ...
        {activeMode === 'login' && (
          <div className="mb-5 p-3.5 rounded-xl bg-slate-900/60 border border-white/5">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`py-1.5 rounded-lg bg-slate-950 text-[9px] font-bold border cursor-pointer hover:bg-slate-900 transition ${color}`}
                >
                  {label}
                </button>
              ))}
          ...
          {activeMode === 'register' && (
            <div>
              ...
              <select
                ...
                className="w-full glass-input px-3.5 py-2.5 rounded-xl text-xs text-white bg-[#0F172A] border border-white/10 focus:border-emerald-500/40 outline-none cursor-pointer"
              >
