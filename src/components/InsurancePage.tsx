...
const QuoteModal: React.FC<QuoteModalProps> = ({ quote, onClose, onConfirm }) => {
 ...
 return (
 <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-md" onClick={e => e.target === e.currentTarget && onClose()}>
 <div className="glass-card max-w-md w-full p-6 rounded-2xl border relative shadow-2xl">
 <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-full text-slate-400 hover:text-white transition cursor-pointer">
 <X className="w-4 h-4" />
 </button>
...
export const InsurancePage: React.FC = () => {
 ...
 <div key={label} className="glass-card p-4 rounded-2xl border text-center">
 ...
 : 'bg-white/5 border text-white hover:bg-emerald-600 hover:border-emerald-600 hover:text-slate-950'
 }`}
 >
 {activated ? (
